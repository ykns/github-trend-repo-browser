...
export function useInfiniteQueryTrendingReposCreatedFrom(
  lastNumberOfDays: number,
  reposPerPage: number = 100
) {
  const fetch = createFetchTrendingReposCreatedFrom(
    lastNumberOfDays,
    reposPerPage
  );
  return useInfiniteQuery(
    'trendingReposCreatedAfterInclusive',
