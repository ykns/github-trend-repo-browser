...
export function PageResultsTable<T>({ pages, getIdFn, columnsDefinitions, children }: PageResultTableProps<T>) {
  const allColumnsDistinctValues: {
    ...
    .reduce(
      (acc, c) => ({
        ...acc,
        [c.label]: new Set<string | null | undefined>(
          pages.flatMap((p) => p.results).map((r) => c.valueFn && c.valueFn(r))
        ),
      }),
      {}
    );
  const [allColumnsSelectedValues, setAllColumnsSelectedValues] = useState<{
      ...
      <thead>
        <tr>
          {columnsDefinitions.map((c) => (
            <th scope="col" style={{ width: c.width }} key={c.label}>
              {!c.filterByValue ? (
                c.label
              ) : (
                <>
        ...
        {pages.map((p) => (
          <Fragment key={p.pageIndex}>
            {p.results.reduce((acc, item) => {
              ...
              if (!isFiltered) {
                acc.push(<tr key={getIdFn(item)}>{...tds}</tr>);
              }
              return acc;
            }, [] as React.ReactNode[])}
          </Fragment>
